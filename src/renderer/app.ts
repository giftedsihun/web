...
type BrowserStore = { load(): Promise<{ bookmarks: Bookmark[]; history: HistoryItem[] }>; toggleBookmark(item: Bookmark): Promise<Bookmark[]>; addHistory(item: HistoryItem): Promise<HistoryItem[]>; indexDocument(item: IndexedDocument): Promise<number>; documentCount(): Promise<number>; graph(): Promise<GraphData>; search(query: string): Promise<{ results: SearchResult[]; error?: string }>; notes(): Promise<Note[]>; saveNote(note: Omit<Note, "id" | "createdAt">): Promise<Note> };
declare global { interface Window { browserStore: BrowserStore } }

type Tab = { id: number; title: string; url: string; view: Electron.WebviewTag; headings?: Array<{ level: string; text: string }>; links?: LinkItem[]; pendingQuote?: string };
...
function renderPanel() {
  ...
  if (panel === "search") { content.innerHTML = `<form id="local-search"><input id="local-query" placeholder="AI AND 논문 NOT 광고" /><button>찾기</button></form><p class="help">SQLite FTS5 문서와 지식 노트를 함께 검색합니다.<br>AND · OR · NOT · 괄호 · "구문 검색" 지원</p><div id="search-results"></div>`; byId<HTMLFormElement>("local-search").onsubmit = async (event) => { event.preventDefault(); const query = byId<HTMLInputElement>("local-query").value.trim(); const response = await window.browserStore.search(query); const target = byId("search-results"); const list = response.results.map((result) => result.kind === "note" ? `<button class="note-card" data-url="${encodeURIComponent(result.url)}" data-quote="${encodeURIComponent(result.quote)}"><b>NOTE · ${escapeHtml(result.title)}</b><q>${escapeHtml(result.quote)}</q><span class="note-tags">${escapeHtml(result.tags.map((tag) => `#${tag}`).join(" "))}</span></button>` : `<button class="saved" data-url="${encodeURIComponent(result.url)}"><b>${escapeHtml(result.title)}</b><span>${escapeHtml(result.url)}</span></button>`).join(""); target.innerHTML = response.error ? `<p class="help">${escapeHtml(response.error)}</p>` : `<p class="help">${response.results.length}개의 결과</p>${list || '<p class="help">일치하는 색인 문서가 없습니다.</p>'}`; target.querySelectorAll<HTMLElement>("[data-url]").forEach((item) => item.onclick = () => navigate(decodeURIComponent(item.dataset.url || ""))); }; }
  if (panel === "notes") { content.innerHTML = `<p class="help">페이지의 문장을 선택한 뒤 상단의 NOTE +를 누르세요.</p><div id="note-list"><p class="help">노트를 불러오는 중입니다.</p></div>`; window.browserStore.notes().then((notes) => { if (panel !== "notes") return; const list = byId("note-list"); list.innerHTML = notes.length ? notes.map((note) => `<button class="note-card" data-url="${encodeURIComponent(note.sourceUrl)}" data-quote="${encodeURIComponent(note.quote)}"><b>${escapeHtml(note.sourceTitle)}</b><q>${escapeHtml(note.quote)}</q><span class="note-tags">${escapeHtml(note.tags.map((tag) => `#${tag}`).join(" "))}</span></button>`).join("") : `<p class="help">저장된 지식 노트가 없습니다.</p>`; list.querySelectorAll<HTMLElement>(".note-card").forEach((item) => item.onclick = () => { const tab = current(); if (tab) tab.pendingQuote = decodeURIComponent(item.dataset.quote || ""); navigate(decodeURIComponent(item.dataset.url || "")); }); }); }
...
byId("add-note").onclick = async () => { const tab = current(); if (!tab || tab.url.startsWith("about:")) return; try { const quote = await tab.view.executeJavaScript("window.getSelection()?.toString().trim() || ''") as string; if (!quote) { setStatus("SELECT TEXT TO CREATE A NOTE"); return; } noteQuote.textContent = quote.slice(0, 3000); noteBody.value = ""; noteTags.value = ""; noteDialog.showModal(); } catch { setStatus("NOTE CAPTURE UNAVAILABLE"); } };
byId("close-note").onclick = () => noteDialog.close();
byId<HTMLFormElement>("note-form").onsubmit = async (event) => { event.preventDefault(); const tab = current(); const quote = noteQuote.textContent || ""; if (!tab || !quote) return; await window.browserStore.saveNote({ quote, body: noteBody.value, tags: noteTags.value.split(","), sourceUrl: tab.url, sourceTitle: tab.title }); noteDialog.close(); setStatus("KNOWLEDGE NOTE SAVED"); if (panel === "notes" || panel === "graph") renderPanel(); };
